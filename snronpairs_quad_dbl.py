...
def getsnr_forpairs(det):
   lid,imid,uSNR=np.loadtxt("out_fincat_%s/all_lensprop.txt"%(det),usecols=(0,1,7),unpack=True)
   qdid,qdmag,qdtdel,srcqid=np.loadtxt("out_fincat_%s/quadall_imgprop.txt"%(det),usecols=(0,3,4,6),unpack=True)
   dbid,dbmag,dbtdel,srcdid=np.loadtxt("out_fincat_%s/dblall_imgprop.txt"%(det),usecols=(0,3,4,6),unpack=True)
